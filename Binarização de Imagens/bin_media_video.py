...
def draw_window(img):
    ...
    for p in points_original:
        # Desenha os pontos fixos na imagem independente do pan
        sx = int(p[0]*zoom - x0)
        sy = int(p[1]*zoom - y0)
...
def ask_user_options():
    ...
    create_csv = messagebox.askyesno("Opção CSV", "Deseja criar um CSV com percentagem de preto por frame?")
    show_first_frame = messagebox.askyesno("Visualizar Primeiro Frame", "Deseja visualizar o primeiro frame binarizado?")
    root.destroy()
...
if __name__ == "__main__":
    ...
    if len(sys.argv)<2:
        print("Uso: python bin_media_video.py caminho/para/video.mp4")
    else:
        ...
        if not os.path.exists(video_path):
            ...
            def main_video(path):
                ...
                while True:
                    ...
                    black_percentages.append((time_sec, black_percentage))
                    frame_idx += 1
                ...
                if csv_path:
                    with open(csv_path, 'w', newline='') as f:
                        writer = csv.writer(f)
                        writer.writerow(['Tempo (s)', 'Percentagem Preto'])
                        writer.writerows(black_percentages)
                    print(f"CSV salvo em {csv_path}")

                if bw_frames:
                    ResultViewer(bw_frames[0], "Primeiro Frame Binarizado")
